...
from adversarial.attacks import pgd
from adversarial.attacks import apgd_train as apgd
from adversarial.losses import compute_acc, compute_loss, l2, ce, ComputeLossWrapper
import wandb
...
parser.add_argument(
    "--clip_model_name", type=str, default="ViT-L-14", help="ViT-L-14, ViT-B-32"
)
...
parser.add_argument(
    ...
    type=str2bool,
    default=False,
    help="Whether the embedding is normalized",
)
parser.add_argument("--start_step", type=int, default=0, help="Start step for training")
parser.add_argument(
    "--optimizer_state", type=str, default="", help="Optimizer state file path"
)
...
parser.add_argument(
    "--clean_weight", type=float, default=0.0, help="Weight for clean loss"
)
parser.add_argument("--trades", type=str2bool, default=False, help="Use TRADES")
parser.add_argument(
    "--opt", type=str, default="adamw", help="Optimizer type; sgd, adamw"
)
parser.add_argument(
    "--momentum_sgd", type=float, default=0.9, help="Momentum for SGD optimizer"
)
parser.add_argument("--lr", type=float, default=1e-5, help="Learning rate")
parser.add_argument("--wd", type=float, default=1e-4, help="Weight decay")
parser.add_argument(
    "--attack", type=str, default="apgd", help="Adversarial attack type"
)
parser.add_argument(
    "--inner_loss",
    type=str,
    default="l2",
    help="Inner loss function for adversarial training",
)
parser.add_argument(
    "--norm", type=str, default="linf", help="Norm for adversarial perturbation"
)
parser.add_argument(
    "--eps", type=float, default=4, help="Epsilon for adversarial perturbation"
)
parser.add_argument(
    "--iterations_adv", type=int, default=10, help="Iterations for adversarial attack"
)
parser.add_argument(
    "--stepsize_adv",
    type=float,
...
parser.add_argument(
    "--wandb", type=str2bool, default=True, help="Use Weights & Biases for logging"
)
parser.add_argument("--experiment_name", type=str, default="")
parser.add_argument(
    "--overwrite", type=str2bool, default=False, help="Overwrite existing directory"
)
...
parser.add_argument(
    ...
    type=str2bool,
    default=True,
    help="Save 10 training checkpoints",
)
...
#
@app.function(
    image=robust_finetune_image,
    secrets=[wandb_secret],
    volumes={OUT_PATH: out_vol},
    gpu="a100-80gb:4",
    timeout=10 * 24 * 60 * 60,
)
def main(args):
    ...
    def train_one_epoch(
        ...
        for i, (data, targets) in enumerate(dataloader):
            # loss for the attack #?????
            # if i != 0:
            #     # print(f'STEP: {i-1}, TIME TAKEN:{time.time()-step_start_time}')
            ...
            with torch.no_grad():
                embedding_orig = model_orig(
                    vision=data, output_normalize=args.output_normalize
                )
            ...
            loss = compute_loss(
                ...
                targets=targets,
                embedding_orig=(
                    embedding_orig if not args.trades else embedding_clean_no_grad
                ),
                logit_scale=100.0,
            ...
            with torch.no_grad():
                # only for logging
                embedding_orig.cuda()
                cos_sim_clean = F.cosine_similarity(
                    embedding_clean, embedding_orig, dim=1
                ).mean()
                cos_sim = F.cosine_similarity(
                    embedding_adv, embedding_orig, dim=1
                ).mean()
                if is_classification:
            ...
            if (step_total - 1) % args.eval_freq == 0:
                ...
                with torch.no_grad():
                    embedding_adv_eval_norm = model(
                        data_eval_adv, output_normalize=True
                    )  # we set output_normalize to True
                    logits_eval_adv = (
                        embedding_adv_eval_norm @ embedding_text_labels_norm
                    )
                    ...
                    cos_sim_eval = F.cosine_similarity(
                        embedding_adv_eval_norm, embedding_eval_norm, dim=1
                    ).mean()
                eval_logs["eval/racc"] = racc_eval
                eval_logs["eval/acc"] = acc_eval
                eval_logs["eval/cos-sim"] = cos_sim_eval
                print(
                    f"[eval-acc] {acc_eval:.2f} [eval-racc] {racc_eval:.2f} [eval-cos-sim] {cos_sim_eval:.3f}"
                )
                model.train()
                del (
                    data_eval_adv,
                    data_eval,
                    targets_eval,
                    embedding_adv_eval_norm,
                    logits_eval_adv,
                    embedding_eval_norm,
                    logits_eval,
                )
                # eval_end_time=time.time()
                # print(f'EVAL TIME: {eval_end_time-eval_start_time}')
            ...
            if (step_total - 1) % args.log_freq == 0:
                ...
                if (step_total - 1) % (args.log_freq * 10) == 0:
                    ...
                    batch_average_time = (
                        (time.time() - epoch_start_time) / (i + 1) / (60**2)
                    )
                    epoch_average_time = batch_average_time * len(dataloader)
                    this_epoch_remaining = (
                        epoch_average_time - (time.time() - epoch_start_time) / 60**2
                    )
                    total_remaining = epoch_average_time * (
                        args.total_epochs - epoch - i / len(dataloader)
                    )
                    print(
                        f"[epoch average time] {epoch_average_time:.2f} [this epoch remaining] "
            ...
            if args.save_checkpoints and (step_total % (args.steps // 10) == 0):
                ...
                torch.save(
                    unwrap_model(model).model.state_dict(),
    ...
    if args.wandb:
        init_wandb(
            project_name="robust-clip-finetune-modal",
    ...
    with torch.no_grad():
        ...
        for el in (text_tokens[:500], text_tokens[500:]):
            ...
            embedding_text_labels_norm.append(
                model_orig.encode_text(el.to(main_device), normalize=True)
                .detach()
                .cpu()
            )
        embedding_text_labels_norm = torch.cat(embedding_text_labels_norm).T.to(
            main_device
        )
        assert torch.allclose(
            F.normalize(embedding_text_labels_norm, dim=0), embedding_text_labels_norm
        )
    ...
    model_orig = ClipVisionModel(
        model=model_orig.visual, args=args, normalize=normalize
    )
    if num_gpus > 1:
...
def entrypoint():
    ...
    assert (
        args.eval_freq % args.log_freq == 0
    ), "eval_freq must be a multiple of log_freq"
