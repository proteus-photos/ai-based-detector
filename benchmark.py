    ...
    with torch.no_grad():
        ...
        for index in tqdm(img_path_table.index, total=len(img_path_table)):
            ...
            for k in transform_dict:
                batch_img[k].append(
                    transform_dict[k](Image.open(filepath).convert("RGB"))
                )
            batch_id.append(index)

            if (len(batch_id) >= batch_size) or (index == last_index):
                for k in do_transforms:
                    batch_img[k] = torch.stack(
                        batch_img[k], 0
                    )  # convert the list of tensors to a single tensor along axis 0
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        "-w",
        type=str,
        help="The directory to the networks weights",
        default="./weights",
    )
    parser.add_argument(
        "--models",
        ...
        default="clipdet_latent10k_plus,Corvi2023",
    )
    parser.add_argument(
        "--fusion", "-f", type=str, help="Fusion function", default="soft_or_prob"
    )
    parser.add_argument(
        "--device", "-d", type=str, help="Torch device", default="cuda:0"
    )
    parser.add_argument(
        "--batch_size", "-b", type=int, help="No. of images in a batch", default=1
    )
    parser.add_argument(
        "--post_process",
        "-p",
    ...
    if args["fusion"] is not None:
        table["fusion"] = apply_fusion(
            table[args["models"]].values, args["fusion"], axis=-1
        )
